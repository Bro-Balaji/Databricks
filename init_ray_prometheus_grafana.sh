...
if [[ ! -z $DB_IS_DRIVER ]] && [[ $DB_IS_DRIVER = TRUE ]] ; then
  ...
  cd "$BASE_DIR"
  # Install pyspark
  /databricks/python3/bin/pip install -qq py4j pyspark
...
EOL
  ...
  prometheus_jar_path="/Volumes/catalogtest1/bigdata/managed_volume/scripts/prometheus-3.0.1.linux-amd64.tar.gz"
  cp ${prometheus_jar_path} ${BASE_DIR}/
  ...
  grafana_jar_path="/Volumes/catalogtest1/bigdata/managed_volume/scripts/grafana-9.2.4.linux-amd64.tar.gz"
  cp ${grafana_jar_path} ${BASE_DIR}/
